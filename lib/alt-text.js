...
/** Quality check for alt text the user is about to save. */
export function auditAlt(alt) {
  const v = squash(alt);
  if (!v) return { state: "missing", msg: "Alt text missing" };
  if (v.length > ALT_MAX) return { state: "long", msg: `Too long (${v.length}/${ALT_MAX})` };
  if (LEADING_NOISE.test(v)) return { state: "weak", msg: 'Drop the "image of" opener' };
  if (v.length < 10) return { state: "weak", msg: "Too short to be descriptive" };
  return { state: "ok", msg: `Alt text OK (${v.length})` };
}
