 ...
 *
 * @param {object} [opts] - { member } true unlocks everything (paid).
...
export function applyGate(ordered, opts = {}) {
  // A paid Boko membership unlocks everything; a legacy per-domain licence key
  // still does too, so existing customers aren't broken by the move to SSO.
  const ent = opts.entitlement || null;
  const paidPlan = !!(ent && ent.plan && ent.plan !== "free");
  const member = !!opts.member || paidPlan;
